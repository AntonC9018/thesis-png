...
pentru care nu era posibil de creat un comprimator de uz liber atunci\cite{gif_unusable_reason}.
...
Din această cauză, s-a decis să realizeze implementarea unui decodor Zlib de la zero.
...
este dificit să genereze niște date pentru teste.
...
omitând însă separarea modulelor de citire și de scriere pentru a reduce complexitatea.
...
va arăta aproximativ în următorul fel:
...
Din această cauză, când se face procesare a secvenței, implementarea trebuie să fie atentă
să nu le permite să ajungă la noduri.
Aceasta a fost realizat prin înscrierea pozițiilor în noduri înainte de modificare a secvenței.
...
aceste informații nu pot fi transmise direct din parser la codul care folosește parserul pentru
...
După invocare, trebuie să stabilească care noduri pot fi umplute cu informații,
și dacă trebuie să creeze noduri noi.
...
Proprietatea \texttt{level} accesibilă pe orice context crează un \texttt{LevelContext}
...
\unnumberedChapter{Concluzii Finale și Recomandările}
...
dar este proiectat în așa fel ca să permite extinderea, folosirea liberă și
capacitățile neaccesibile în trecut în alte formate.
\ac{PNG} este așa cum este dintr-un motiv.

Formatul de comprimare Zlib folosit în \ac{PNG},
cu toate că nu este însuși folosit pe scară largă în aplicații,
folisește subformatul DEFLATE care este o parte esențială a mai multor alți algoritmi.
DEFLATE folosește arbori Huffman care pot fi utile pentru mai multe aplicații din diferite domenii
unde e necesar un fel de codificare.

A fost discutat procesul de dezvoltare unei aplicații ce ține de folosire a unui arbore sintactic
pentru a putea vizualiza grafic niște date despre \ac{PNG}.
Folosirea unui arbore a fost argumentată, iar implementarea imbunătățită în mod gradual și discutată pe larg.
