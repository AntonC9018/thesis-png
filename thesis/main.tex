...
\newcommand{\authorName}{Curmanschii Anton} % NUME PRENUME; Fără patronimic
...
\newcommand{\conducatorName}{Pereteatcu Sergiu} % NUME PRENUME

\newcommand{\authorNameRu}{Курманский Антон Александрович} % ФАМИЛИЯ, ИМЯ, ОТЧЕСТВО
...
\textbf{la \thesisType{} \textquote{\thesisTitle{}}, a studentului \authorName{}, grupa \uniGroupName{}, programul de studii \programulDeStudii.}
...
\textbf{of the \thesisType{} thesis  \textquote{\thesisTitleEng} of the student \authorName{}, group \uniGroupName{}, \programulDeStudii{} study program.}
...
\ac{PNG} este un format adaptiv și extensibil, având o structură bazată pe chunk-uri, versionate separat unul
...
acest format, a extinde formatul prin niște chunk-uri personalizate, și pentru a afla despre
...
Lucrarea dată are un scop educațional -- informarea despre structura formatului \acs{PNG} și a
formatului integrat de comprimare Zlib.
Pentru aceasta s-a decis să fie creată o interfață grafică care
...
Pentru implementarea formatului DEFLATE a fost folosit, ca referință, într-o măsură scăzută codul librăriei uzlib \cite{gzip_impl}.
Pentru a depana implementarea DEFLATE s-a mai folosit exemplul aplicării algoritmului DEFLATE și datele test ale Google \cite{deflate_tests}.
A fost folosită librăria Raylib \cite{raylib} drept librărie grafică.
...
Pentru a crea aplicația sunt suficiente documentele de specificație a formaturilor necesare,
...
Scopul tezei nu constă nici în reprezentarea unei imagini pe ecran ca la alte librării,
nici în afișarea unei imagini pe ecran,
...
\section{Descriere generală a formatului de fișier \acs{PNG}}

\subsection{Introducere}

Formatul de fișier \ac{PNG} este unul din cele mai răspândite formaturi de imagine.
El este folosit în numeroase domenii pe scară largă, incluzând majoritatea aplicațiilor
web, aplicații grafice care salvează sau încarcă imagini din fișiere, precum și în dezvoltarea de jocuri.
...
Fraza \textquote{fără pierderi} reprezintă faptul că pixelii imaginii
pot fi comprimați într-un fișier \ac{PNG},
după ce decomprimați din acest fișier de către cititor,
...
pentru care nu era posibil de creat un comprimator de uz liber atunci\cite[12.2.]{png_spec}\cite{gif_unusable_reason}.
În plus, inginerii care au inventat \ac{PNG} au străduit să îmbunătățească algoritmul de comprimare,
precum și structura formatului nou, adăugând unele capacități inaccesibile în \ac{GIF}
ca posibilitate de a comprima imagini în culori reale (truecolor images),
posibilitate de a folosi un canal separat pentru transparența,
...
sunt specificate ca \textquote{undefined behavior} în C!
...
programul va da eroarea (ar \textquote{panica}) cu stack trace-ul execuției.
...
\begin{itemize}
    \item 
        ...
        dacă această versiune nu este în lista lor a \textquote{pachetelor de încredere},
    ...
    \item
        ...
        Compilarea aplicațiilor pe Windows mereu implică \textquote{probleme pe cap}.
...
Pentru a înțelege problema mai bine, ar fi benefic să înțeleagă cum funcționează algoritmul Zlib.
În plus, deoarece algorimul face parte din standardul \ac{PNG},
și structura acestuia poate fi decodificată și analizată,
ar fi benefic să realizeze un fel de parser special pentru Zlib pentru a putea vizualiza nodurile Zlib
în mod bogat în interfață.
Din acest motiv, s-a decis să realizeze un decodor Zlib de la zero.
...
În structura finală a aplicației, poziția pe biți a componentelor din fluxul de date se salvează la crearea nodurilor.
...
\begin{itemize}
    ...
    \item
        ...
        a \textquote{mișca} poziția de început curentă la poziția de început 
...
while (true)
{
    ...
    // Apelări la reader.read() nu mai sunt valide.
    if (readResult.isEnd)
    {
...
Operația de \textquote{mișcare} a secvenței este destul de simplă -- se suprascrie poziția de început a secvenței.
...
este posibil de a \textquote{deturna} o secvență, definind, de exemplu, segmentul propriu de început.
...
\section{Oportunitate de abstractizare a parser-ului}
...
\subsection{Descrierea problemei de \textquote{sărituri}}

Posibilitatea de a putea păstra informațiile despre originea fiecărui \textquote{nod},
...
atunci când fluxul de date poate \textquote{sări}, adică atunci când pozițiile absolute
...
Atunci ar trebui să adauge și un fel de \textquote{comsumed position hint} la cititor,
